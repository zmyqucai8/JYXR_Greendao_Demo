...
public class WenJianFragment extends Fragment implements View.OnClickListener {
    ...
    List<FileMsgRecordBean> fileMsgRecordList;
    FileAdapter adapter;

    public void initData() {
        //获取当前账户消息数据
        AccountBean currentAccount = DaoUtils.getCurrentAccount();
        fileMsgRecordList = currentAccount.getFileMsgRecordList();
        if (null == fileMsgRecordList) {
            fileMsgRecordList = new ArrayList<>();
        }
        adapter = new FileAdapter(this, fileMsgRecordList);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_remove_all:
                //移除全部
                if (adapter.getData().size() > 0 && DaoUtils.removeFileMsg(adapter.getData())) {
            ...
            case R.id.btn_remove_last:
                //移除最后一条
                if (adapter.getData().size() > 0 && DaoUtils.removeFileMsg(adapter.getData().get(adapter.getData().size() - 1))) {
                    ToastUtils.showShort("移除成功");
            ...
            case R.id.btn_add:
                //添加一条
                FileMsgRecordBean msgRecordBean = TestUtlis.createFileMsg();
                if (DaoUtils.addFileMsg(msgRecordBean)) {
                    adapter.addData( msgRecordBean);
