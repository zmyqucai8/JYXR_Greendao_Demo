...
public class XiangMuFragment extends Fragment implements View.OnClickListener {
    ...
    public void initData() {
        ...
        projectList = currentAccount.getProjectList();
        if (null == projectList) {
            projectList = new ArrayList<>();
        }
        adapter = new XiangMuAdapter(this, projectList);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_remove_all:
                //移除全部
                if (adapter.getData().size() > 0 && DaoUtils.removeProject(adapter.getData())) {
            ...
            case R.id.btn_remove_last:
                //移除最后一条
                if (adapter.getData().size() > 0 && DaoUtils.removeProject(adapter.getData().get(adapter.getData().size() - 1))) {
                    ToastUtils.showShort("移除成功");
            ...
            case R.id.btn_add:
                ...
                ProjectBean projectBean = TestUtlis.createProject();
                if (DaoUtils.addProject(projectBean)) {
                    adapter.addData(projectBean);
