...
public class YongHuFragment extends Fragment implements View.OnClickListener {
    ...
    YongHuAdapter adapter;

    public void initData() {
        //获取当前账户消息数据

        currentAccount = DaoUtils.getCurrentAccount();
        currentAccount.resetUserList();
        userList = currentAccount.getUserList();
        if (null == userList) {
            userList = new ArrayList<>();
        }

        adapter = new YongHuAdapter(this, userList);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_remove_all:
                //移除全部
                if (adapter.getData().size() > 0 && DaoUtils.removeUser(adapter.getData())) {
                    ToastUtils.showShort("移除成功");
                    adapter.getData().clear();
                    adapter.notifyDataSetChanged();
                    currentAccount.resetHaoYouList();
                } else {
            ...
            case R.id.btn_remove_last:
                //移除最后一条
                if (adapter.getData().size() > 0 && DaoUtils.removeUser(adapter.getData().get(adapter.getData().size() - 1))) {
                    ToastUtils.showShort("移除成功");
            ...
            case R.id.btn_add:
                //添加一条
                UserBean userBean = TestUtlis.createUser(false);
                if (DaoUtils.addUser(userBean)) {
