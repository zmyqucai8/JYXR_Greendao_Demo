...
public class CaoGaoFragment extends Fragment implements View.OnClickListener {
    ...
    List<DraftMsgRecordBean> draftMsgRecordList;
    CaoGaoAdapter adapter;
    ...
    public void initData() {
        ...
        adapter = new CaoGaoAdapter(this, draftMsgRecordList);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_remove_all:
                //移除全部
                if (adapter.getData().size() > 0 && DaoUtils.removeDraftMsg(adapter.getData())) {
            ...
            case R.id.btn_remove_last:
                //移除最后一条
                if (adapter.getData().size() > 0 && DaoUtils.removeDraftMsg(adapter.getData().get(adapter.getData().size() - 1))) {
                    ToastUtils.showShort("移除成功");
            ...
            case R.id.btn_add:
                //添加一条
                DraftMsgRecordBean draftMsgRecordBean = TestUtlis.createCaoGaoMsg();
                if (DaoUtils.addDraftMsg(draftMsgRecordBean)) {
                    adapter.addData(draftMsgRecordBean);
